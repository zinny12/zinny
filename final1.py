...
###############################3
Materials = ['SK머티리얼즈', '한솔케미칼', '솔브레인', '동진쎄미켐', '포토레지스트', '이엔에프테크놀로지', '덕산테코피아', '백광산업', '경인양행', '디엔에프', '오션브릿지', '와이씨켐', '지오엘리먼트', '엘티씨', '코미코', '후성', '티이엠씨', '레이크머티리얼즈', '원익머티리얼즈', '제이아이테크']
...
Parts = ['티씨케이', '하나머티리얼즈', '윌덱스', '비씨엔씨', '케이엔제이', '에스앤에스텍', '펠리클', 'pelicle', '에프에스티', '아스플로', '한솔아이원스', '뉴파워프라즈마', '원익QnC', '메카로', '미코']
...
Equipments = ['원익IPS', '유진테크', '주성엔지니어링', 'ALD', '테스', 'HPSP', '파크시스템스', '넥스틴', '오로스테크놀로지', '케이씨텍', '피에스케이', '에이피티씨', '와이아이케이', '엘오티베큠', '싸이맥스', '제우스', '디바이스이엔지', '저스템', '원익홀딩스',  '에스티아이', '씨앤지하이테크', 'GST', '유니셈', '지앤비에스엔지니어링']
...
OSAT = ['SFA반도체', '하나마이크론', '네패스', '엘비세미콘', '한양디지텍', '아이텍', '시그네틱스', '윈팩', '두산테스나', '네패스아크', '에이팩트', '큐알티']
...
PackagingMaterials = ['심텍', '아비코전자', 'DDR5', '해성디에스', '엠케이전자', '덕산하이메탈']

value_chain = "[Packaging Materials]" + "\n"

for company in PackagingMaterials:
    summary = ""
    for n in range(1,41,10) : 
        response = requests.get('https://search.naver.com/search.naver?where=news&sm=tab_opt&sort=0&pd=4&ds=&query='+company+'&start='+str(n))
        soup = bs(response.text, 'html.parser')
...
Packaging_Materials_value_chain = value_chain
...
Distribution_value_chain = value_chain

combined = materials_value_chain + "\n" + equipments_value_chain + "\n" +  parts_value_chain + "\n" + OSAT_value_chain + "\n" + Packaging_Materials_value_chain + "\n" + test_parts_value_chain + "\n" + Distribution_value_chain
